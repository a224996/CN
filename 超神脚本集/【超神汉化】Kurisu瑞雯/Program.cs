...
namespace KurisuRiven
{
    internal class Program
    {
        ...
        {
            _config = new Menu("【超神汉化】K-瑞雯", "kurisuriven", true);

            var tsMenu = new Menu("目标选择", "selector");
            TargetSelector.AddToMenu(tsMenu);
            _config.AddSubMenu(tsMenu);

            var owMenu = new Menu("走砍", "Orbwalk");
            _orbwalker = new Orbwalking.Orbwalker(owMenu);
            owMenu.AddItem(new MenuItem("fleemode", "逃跑")).SetValue(new KeyBind(65, KeyBindType.Press));
            _config.AddSubMenu(owMenu);

            var drMenu = new Menu("显示", "drawings");
            drMenu.AddItem(new MenuItem("drawrange", "W范围")).SetValue(true);
            drMenu.AddItem(new MenuItem("drawpassive", "显示被动")).SetValue(true);
            drMenu.AddItem(new MenuItem("drawengage", "显示模式")).SetValue(true);
            drMenu.AddItem(new MenuItem("drawkill", "显示可杀")).SetValue(true);
            drMenu.AddItem(new MenuItem("drawtarg", "显示目标线圈")).SetValue(true);
            _config.AddSubMenu(drMenu);

            var cMenu = new Menu("连招", "combo");
            cMenu.AddItem(new MenuItem("engage", "模式"))
                .SetValue(new StringList(new[] { "E->R->提亚马特->W", "E->提亚马特->R->W" }));
            cMenu.AddItem(new MenuItem("reckless", "连招直接开R")).SetValue(new KeyBind(88, KeyBindType.Toggle));
            cMenu.AddItem(new MenuItem("multir1", "大招击中人数 >=")).SetValue(new Slider(1, 1, 5));
            cMenu.AddItem(new MenuItem("multir2", "大招如果伤害>%= ")).SetValue(new Slider(50, 1, 99));
            cMenu.AddItem(new MenuItem("autow", "(W)击中人数 >=")).SetValue(new Slider(2, 2, 5));
            ...
            menuQ.AddItem(new MenuItem("qint", "打断使用")).SetValue(true);
            menuQ.AddItem(new MenuItem("prediction", "预判移动")).SetValue(true);
            menuQ.AddItem(new MenuItem("keepq", "保持顺势斩")).SetValue(true);
            ...
            menuW.AddItem(new MenuItem("antigap", "防突使用")).SetValue(true);
            menuW.AddItem(new MenuItem("wint", "打断使用")).SetValue(true);
            ...
            menuE.AddItem(new MenuItem("vhealth", "血量控制")).SetValue(new Slider(40, 1));
            sMenu.AddSubMenu(menuE);

            var menuR = new Menu("R", "blade");
            menuR.AddItem(new MenuItem("user", "连招使用")).SetValue(true);
            menuR.AddItem(new MenuItem("usews", "使用二段R")).SetValue(true);
            menuR.AddItem(new MenuItem("checkover", "检查伤害过度")).SetValue(false);
            sMenu.AddSubMenu(menuR);

            _config.AddSubMenu(sMenu);

            var mMenu = new Menu("杂项", "misc");
            mMenu.AddItem(new MenuItem("useignote", "使用点燃")).SetValue(true);
            mMenu.AddItem(new MenuItem("useitems", "使用九头蛇/幽梦")).SetValue(true);
            mMenu.AddItem(new MenuItem("forceaa", "清线强行平A")).SetValue(false);
            _config.AddSubMenu(mMenu);

            var dMenu = new Menu("调试", "debug");
            dMenu.AddItem(new MenuItem("debugtrue", "真实范围")).SetValue(false);
            dMenu.AddItem(new MenuItem("debugdmg", "连招伤害")).SetValue(false);
            _config.AddSubMenu(dMenu);
			
            var QMenu = new Menu("超神汉化", "Chaoshen");
            QMenu.AddItem(new MenuItem("ChaoshenQ", "L#汉化群：386289593"));
