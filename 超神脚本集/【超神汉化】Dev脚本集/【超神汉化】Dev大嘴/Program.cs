    ...
    class Program
    {
        ...
        static void AssemblyUtil_onGetVersionCompleted(OnGetVersionCompletedArgs args)
        {
            if (args.LastAssemblyVersion == Assembly.GetExecutingAssembly().GetName().Version.ToString())
                Game.PrintChat(string.Format("<font color='#fb762d'>Dev澶у槾 鏄渶鏂扮増鏈副.</font>"));
            else
                Game.PrintChat(string.Format("<font color='#fb762d'>Dev澶у槾 鏈夋柊鐗堟湰锛佽閫氱煡鐒＄偤杩涜鏇存柊! {0}</font>", args.LastAssemblyVersion));
        }
        ...
        private static void InitializeMainMenu()
        {
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("RMaxStacksCombo", "R最大层数").SetValue(new Slider(3, 1, 5)));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseIgnite", "使用点燃").SetValue(true));
            Config.SubMenu("Combo").AddItem(new MenuItem("EManaCombo", "E蓝量控制").SetValue(new Slider(20, 1, 100)));
            ...
            Config.SubMenu("Harass").AddItem(new MenuItem("UseRHarass", "使用R").SetValue(true));
            Config.SubMenu("Harass").AddItem(new MenuItem("RMaxStacksHarass", "R最大层数").SetValue(new Slider(1, 1, 5)));
            ...
            Config.SubMenu("KillSteal").AddItem(new MenuItem("QKillSteal", "Q抢人头").SetValue(true));
            Config.SubMenu("KillSteal").AddItem(new MenuItem("RKillSteal", "R抢人头").SetValue(true));

            Config.AddSubMenu(new Menu("偷野", "JungleSteal"));
            Config.SubMenu("JungleSteal").AddItem(new MenuItem("JungleStealAlert", "偷野警报").SetValue(true));
            Config.SubMenu("JungleSteal").AddItem(new MenuItem("JungleSteal", "使用R").SetValue(true));

            Config.AddSubMenu(new Menu("杂项", "Misc"));
            Config.SubMenu("Misc").AddItem(new MenuItem("ChaseEnemyAfterDeath", "死后追人").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("PacketCast", "封包").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("TriforceManagement", "三相控制").SetValue(true));

            Config.AddSubMenu(new Menu("防突", "GapCloser"));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("BarrierGapCloser", "屏障防突").SetValue(true));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("BarrierGapCloserMinHealth", "屏障血量").SetValue(new Slider(40, 0, 100)));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("EGapCloser", "E防突").SetValue(true));
            Config.SubMenu("GapCloser").AddItem(new MenuItem("EInterrupt", "E打断").SetValue(true));

            Config.AddSubMenu(new Menu("大招", "Ultimate"));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("UseAssistedUlt", "使用大招助手").SetValue(true));
            Config.SubMenu("Ultimate").AddItem(new MenuItem("AssistedUltKey", "热键").SetValue((new KeyBind("R".ToCharArray()[0], KeyBindType.Press))));
            ...
            Config.SubMenu("Drawings").AddItem(new MenuItem("RDamage", "R伤害").SetValue(true));

            SkinManager.AddToMenu(ref Config);

            levelUpManager.AddToMenu(ref Config);

            Config.AddSubMenu(new Menu("超神汉化", "System"));
            Config.SubMenu("System").AddItem(new MenuItem("QSystem", "L#汉化群：386289593"));
