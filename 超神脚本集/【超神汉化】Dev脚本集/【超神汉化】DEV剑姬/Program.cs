    ...
    public class Program
    {
        ...
        static void AssemblyUtil_onGetVersionCompleted(OnGetVersionCompletedArgs args)
        {
            if (args.LastAssemblyVersion == Assembly.GetExecutingAssembly().GetName().Version.ToString())
                Game.PrintChat(string.Format("<font color='#fb762d'>Dev鍓戝К 鏄渶鏂扮増鏈副.</font>"));
            else
                Game.PrintChat(string.Format("<font color='#fb762d'>Dev鍓戝К 鏈夋柊鐗堟湰锛佽閫氱煡鐒＄偤杩涜鏇存柊! {0}</font>", args.LastAssemblyVersion));
        }
        ...
        {
            Config = new Menu("【超神汉化】DEV剑姬", "DevFiora", true);
            ...
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRMinHit", "R击中人数").SetValue(new Slider(2, 0, 5)));
            Config.SubMenu("Combo").AddItem(new MenuItem("UseRMinKill", "R可杀人数").SetValue(new Slider(1, 0, 5)));
            ...
            Config.SubMenu("Harass").AddItem(new MenuItem("ManaHarass", "蓝量控制").SetValue(new Slider(30, 1, 100)));
            ...
            Config.SubMenu("Misc").AddItem(new MenuItem("UseWAgainstAA", "敌人平A使用W").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("UseWAfterAttack", "攻击之后使用W").SetValue(true));
            Config.SubMenu("Misc").AddItem(new MenuItem("UseEAfterAttack", "攻击之后使用E").SetValue(true));
