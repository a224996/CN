...
namespace PerfectWard
{
    internal class Program
    {
        ...
        private static string WelcomeMsg = ("<font color = '#cc33cc'>瀹岀編鐪间綅</font><font color='#FFFFFF'> by Da'ath.</font> <font color = '#66ff33'> ~~ LOADED ~~</font>");
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Menu = new Menu("【無爲汉化】完美眼位", "menu", true);
            Menu.AddItem(new MenuItem("on", "启用").SetValue(true));
            Menu.AddItem(new MenuItem("on1", "只显示英雄附近眼位").SetValue(true));
            Menu.AddItem(new MenuItem("on2", "显示范围").SetValue(new Slider(1500, 600, 2000)));
            Menu.AddItem(new MenuItem("on3", "显示站位眼位").SetValue(true));
            Menu.AddItem(new MenuItem("on4", "显示自动行走范围").SetValue(true));
            Menu.AddItem(new MenuItem("key", "启用自动插眼").SetValue(new KeyBind("Z".ToArray()[0], KeyBindType.Press)));
            Menu.AddItem(new MenuItem("key1", "自动插眼到粉红色圈里").SetValue(new KeyBind("A".ToArray()[0], KeyBindType.Press)));
