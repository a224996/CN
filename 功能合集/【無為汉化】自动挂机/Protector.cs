...
namespace Support
{
    ...
    internal class Protector
    {
        ...
        {
            Menu = new Menu("【無為汉化】自动挂机: 保护器", "Protector", true);

            // Marksman Potion Manager
            new PotionManager(Menu);

            // detector
            var detector = Menu.AddSubMenu(new Menu("探测器", "Detector"));

            // detector targeted
            var targeted = detector.AddSubMenu(new Menu("针对性的", "Targeted"));
            ...
            // detector skillshots
            var skillshot = detector.AddSubMenu(new Menu("技能镜头", "Skillshots"));
            skillshot.AddItem(new MenuItem("SkillshotsActive", "启用").SetValue(true));
            skillshot.AddItem(new MenuItem("IsAboutToHitTime", "要打时间").SetValue(new Slider(200, 0, 400)));
            foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsAlly))
            {
                skillshot.AddItem(
