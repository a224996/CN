	...
	internal class ObjectTimer : Base
	{
		...
		private void Drawing_OnEndScene(EventArgs args)
		{
			try
			{
				...
				{
					var text = ((int) (objectMinion.NextRespawnTime - Game.Time)).ToString(CultureInfo.InvariantCulture);
					var pos = objectMinion.MinimapPosition;
				...
				{
					var text = ((int) (objectBarracks.NextRespawnTime - Game.Time)).ToString(CultureInfo.InvariantCulture);
					var pos = objectBarracks.MinimapPosition;
					Class.Helper.DrawText(_miniMapFont,text,(int) pos.X, (int) pos.Y - 8, Color.White);
				}
			}
			catch (Exception ex)
