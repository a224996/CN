...
namespace meta_Smite
{
    class Program
    {
        ...
        private static void Game_OnGameStart(EventArgs args)
        {
            ...
            setSmiteSlot();
            Config = new Menu("【無為汉化】自动惩戒", "metaSmite", true);
        ...
        {
            Config.AddSubMenu(new Menu("野怪", "Camps"));
            if(Game.MapId == GameMapId.TwistedTreeline)
            {
                Config.SubMenu("Camps").AddItem(new MenuItem("TT_Spiderboss", "大蜘蛛").SetValue(true));
                Config.SubMenu("Camps").AddItem(new MenuItem("TT_NGolem", "石像").SetValue(true));
	    ...
	    if (Game.MapId == (GameMapId)11)
	    {
				...
				Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Blue", "蓝Buff").SetValue(true));
				Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Red", "红Buff").SetValue(true));
                //end
                Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Gromp", "癞蛤蟆").SetValue(false));
                Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Murkwolf", "黑暗狼").SetValue(false));
                Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Krug", "石头人").SetValue(false));
                Config.SubMenu("Camps").AddItem(new MenuItem("SRU_Razorbeak", "鸟嘴兽").SetValue(false));
                Config.SubMenu("Camps").AddItem(new MenuItem("Sru_Crab", "螃蟹").SetValue(false));
			}
        }
