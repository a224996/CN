...
namespace MasterActivator
{
    internal class MActivator
    {
        ...
        MItem smiteGanker = new MItem("SmiteGanker", "蓝色打野刀-gank", "s5_summonersmiteplayerganker", 0, ItemTypeId.OffensiveSpell, 750);
         ...
         // Auto shields 
        MItem blackshield = new MItem("BlackShield", "堕落天使", "bShield", 0, ItemTypeId.TeamAbility, 750, SpellSlot.E);
        ...
        MItem bulwark = new MItem("GalioBulwark", "哨兵", "bulwark", 0, ItemTypeId.TeamAbility, 800, SpellSlot.W);
        ...
        MItem inspire = new MItem("KarmaSolKimShield", "天启", "inspire", 0, ItemTypeId.TeamAbility, 800, SpellSlot.E);
        ...
        MMinion wolf = new MMinion("SRU_Murk wolf", "大型暗影狼", 41, 74);
        MMinion razor = new MMinion("SRU_Razorbeak", "深红锋喙鸟", 39, 74); // Ghosts
        MMinion krug = new MMinion("SRU_Krug", "远古石甲虫", 38, 80);
        MMinion crab = new MMinion("Sru_Crab", "螃蟹", 43, 62);
        MMinion gromp = new MMinion("SRU_Gromp", "魔沼蛙", 32, 87); // Ghost

        MMinion tVilemaw = new MMinion("TT_Spiderboss", "巨型蜘蛛", 45, 67);
        MMinion tWraith = new MMinion("TT_NWraith", "幽灵", 45, 67);
        MMinion tGolem = new MMinion("TT_NGolem", "石像", 45, 67);
        ...
        private void createMenuItem(MItem item, String parent, int defaultValue = 0, bool mana = false, int minManaPct = 0)
        {
            if (item.type == ItemTypeId.Ability || item.type == ItemTypeId.TeamAbility)
            {
                ...
                if (abilitySlot != SpellSlot.Unknown && abilitySlot == item.abilitySlot)
                {
                    ...
                    menu.AddItem(new MenuItem(item.menuVariable, "启用").SetValue(true));
                    menu.AddItem(new MenuItem(item.menuVariable + "UseOnPercent", "血量%")).SetValue(new Slider(defaultValue, 0, 100));
                    if (minManaPct > 0)
                    {
                        menu.AddItem(new MenuItem(item.menuVariable + "UseManaPct", "蓝量%")).SetValue(new Slider(minManaPct, 0, 100));
                    }
            ...
            else
            {
                ...
                if (defaultValue != 0)
                {
                    ...
                    {
                        menu.AddItem(new MenuItem(item.menuVariable + "UseXUnits", "几个敌人使用")).SetValue(new Slider(defaultValue, 1, 5));
                    }
                    else
                    {
                        menu.AddItem(new MenuItem(item.menuVariable + "UseOnPercent", "剩余 " + (mana == false ? "%血量" : "%蓝量"))).SetValue(new Slider(defaultValue, 0, 100));
                    }
                }
                Config.SubMenu(parent).AddSubMenu(menu);
            }
        ...
        private void createMenu()
        {
            ...
            Config.SubMenu("purify").AddItem(new MenuItem("dehancer", "减速")).SetValue(false);
            Config.SubMenu("purify").AddItem(new MenuItem("zedultexecute", "劫大招")).SetValue(true);
            ...
            var menuSmiteMobs = new Menu("野怪", "smiteMobs");
            if (Utility.Map.GetMap().Type.Equals(Utility.Map.MapType.TwistedTreeline))
            {
                menuSmiteMobs.AddItem(new MenuItem("TT_Spiderboss", "蜘蛛")).SetValue(true);
                menuSmiteMobs.AddItem(new MenuItem("TT_NWraith", "幽灵")).SetValue(false);
                menuSmiteMobs.AddItem(new MenuItem("TT_NGolem", "石像")).SetValue(true);
                menuSmiteMobs.AddItem(new MenuItem("TT_NWolf", "三狼")).SetValue(true);
            }
            else
            {
            ...
            Config.AddSubMenu(new Menu("防止AOE", "offAOE"));
            ...
            Config.SubMenu("deffensive").AddItem(new MenuItem("justPred", "只是预测")).SetValue(true);
            Config.SubMenu("deffensive").AddItem(new MenuItem("useRecalling", "使用回城")).SetValue(false);
            ...
            Config.AddItem(new MenuItem("predict", "预测伤害")).SetValue(true);
            //Config.AddItem(new MenuItem("enabled", "Enabled")).SetValue(true);
            //38
            Config.AddItem(new MenuItem("enabled", "启用")).SetValue(new KeyBind("L".ToCharArray()[0], KeyBindType.Toggle, true));
