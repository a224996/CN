...
namespace LeagueSharp.Common
{
    public class TargetSelector
    {
        ...
        {
            _configMenu = config;
            config.AddItem(new MenuItem("FocusSelected", "选中目标画圈").SetShared().SetValue(true));
            config.AddItem(new MenuItem("ForceFocusSelected", "选定的目标攻击").SetShared().SetValue(false));
            config.AddItem(
            ...
            config.AddItem(
                new MenuItem("TargetingMode", "优先目标模式").SetShared()
                    .SetValue(
                        new StringList(
                            new[]
                            {
