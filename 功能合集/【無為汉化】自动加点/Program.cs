    ...
    internal static class Program
    {
        ...
        private static void OnGameLoad(EventArgs args)
        {
            ...
            foreach (var entry in DefaultSpellSlotPriorities)
            {
                ...
                subMenu.AddItem(MakeSlider(entry.Key + "extra", "几级开始加点 ?", 1, 1, 18));
                _menu.AddSubMenu(subMenu);
            }

            _activate = new MenuItem("activate", "几级开始启用?").SetValue(new StringList(new[] { "2", "3", "4" }));
            _debug = new MenuItem("debug", "聊天框通知(本地)").SetValue(false);
            _delay = new MenuItem("delay", "加点延迟").SetValue(new Slider(0, 0, 2000));
            ...
            Game.OnUpdate += GameOnOnGameUpdate; //Temp until levelup packet is fixed
            Print("Loaded!");
        }
