...
namespace Ezreal
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            SpellList.Add(R1);
            //Create the menu
            Config = new Menu("【無為汉化】O-伊泽瑞尔", ChampionName, true);
            ...
            Config.SubMenu("Iteams").AddItem(new MenuItem("mura", "自动使用魔宗利刃").SetValue(true));
            Config.SubMenu("Iteams").AddItem(new MenuItem("stack", "自动叠眼泪").SetValue(false));
            Config.SubMenu("Iteams").AddItem(new MenuItem("pots", "使用药瓶").SetValue(true));

            Config.SubMenu("E config").AddItem(new MenuItem("AGC", "E 防突进").SetValue(true));
            Config.SubMenu("E config").AddItem(new MenuItem("smartE", "智能 E 热键").SetValue(new KeyBind('t', KeyBindType.Press))); //32 == space
            ...
            Config.SubMenu("R option").AddItem(new MenuItem("Rcc", "R 被控住的").SetValue(true));
            Config.SubMenu("R option").AddItem(new MenuItem("Raoe", "R 消耗").SetValue(true));
            Config.SubMenu("R option").AddItem(new MenuItem("hitchanceR", "R 非常高命中率").SetValue(true));
            Config.SubMenu("R option").AddItem(new MenuItem("useR", "智能 R 热键").SetValue(new KeyBind('t', KeyBindType.Press))); //32 == space

            Config.SubMenu("Draw").AddItem(new MenuItem("noti", "显示通知").SetValue(false));
            ...
            Config.SubMenu("Draw").AddItem(new MenuItem("onlyRdy", "显示技能CD").SetValue(true));
            Config.SubMenu("Draw").AddItem(new MenuItem("orb", "走砍目标").SetValue(true));
            Config.SubMenu("Draw").AddItem(new MenuItem("qTarget", "Q 目标").SetValue(true));
            Config.SubMenu("Draw").AddItem(new MenuItem("semi", "智能 R 目标").SetValue(false));

            Config.AddItem(new MenuItem("farmQ", "Q 补兵").SetValue(true));
            Config.AddItem(new MenuItem("haras", "骚扰再补兵").SetValue(true));
            Config.AddItem(new MenuItem("wPush", "W 队友 (推塔)").SetValue(true));
            Config.AddItem(new MenuItem("noob", "青铜模式(菜鸟击杀)").SetValue(false));
            Config.AddItem(new MenuItem("Hit", "技能命中率").SetValue(new Slider(2, 3, 0)));
        ...
        {  
            ...
            if ((!t.IsValidTarget() || ObjectManager.Player.UnderTurret(false)) && !Q.IsReady())
               return;

            foreach (var minion in allMinionsQ)
            {
                float predictedHealth = minion.Health;
                if (predictedHealth < minion.TotalAttackDamage)
                {
                    continue;
                }
                else if (!Orbwalking.InAutoAttackRange(minion) && predictedHealth < Q.GetDamage(minion))
                    Q.Cast(minion);
                else if (
