...
namespace Graves_OnKeyToWin
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            SpellList.Add(R1);
            //Create the menu
            Config = new Menu("【無為汉化】O-男枪", ChampionName, true);
            ...
            Config.AddItem(new MenuItem("smartE", "半手动 E 按键").SetValue(new KeyBind('t', KeyBindType.Press))); //32 == space
            Config.AddItem(new MenuItem("AGC", "防突进").SetValue(true));
            Config.AddItem(new MenuItem("Hit", "Q 命中率").SetValue(new Slider(2, 3, 0)));
            Config.AddItem(new MenuItem("HitR", "R 命中率").SetValue(new Slider(2, 2, 0)));
