...
namespace Caitlyn
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            SpellList.Add(R1);
            //Create the menu
            Config = new Menu("【無為汉化】O-女警", ChampionName, true);
            ...
            Config.AddItem(new MenuItem("autoQ", "减少 Q 使用").SetValue(true));
            Config.AddItem(new MenuItem("Hit", "Q 命中率").SetValue(new Slider(2, 3, 0)));
            Config.AddItem(new MenuItem("useR", "半手动 R 按键").SetValue(new KeyBind('t', KeyBindType.Press))); //32 == space
            #region E
                Config.SubMenu("E Config").AddItem(new MenuItem("autoE", "自动 E").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("opsE", "对传送的敌人使用E").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("AGC", "防突进 E").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("useE", "半自动 E 按键").SetValue(new KeyBind('t', KeyBindType.Press)));
