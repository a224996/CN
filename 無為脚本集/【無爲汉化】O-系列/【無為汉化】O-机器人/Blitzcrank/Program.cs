...
namespace Blitzcrank
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            SpellList.Add(R);
            //Create the menu
            Config = new Menu("【無為汉化】O-机器人", ChampionName, true);
            ...
            Config.AddItem(new MenuItem("pots", "使用药瓶").SetValue(true));
            Config.AddItem(new MenuItem("autoW", "自动 W").SetValue(true));
            Config.AddItem(new MenuItem("autoE", "自动 E").SetValue(true));
            Config.SubMenu("Q option").AddItem(new MenuItem("qCC", "自动 Q 硬控与冲刺的敌人").SetValue(true));
            Config.SubMenu("Q option").AddItem(new MenuItem("Hit", "Q 命中率").SetValue(new Slider(2, 3, 0)));
            Config.SubMenu("Q option").AddItem(new MenuItem("minGrab", "最小范围 Q").SetValue(new Slider(250, 125, (int)Q.Range)));
            Config.SubMenu("Q option").AddItem(new MenuItem("maxGrab", "最大范围 Q").SetValue(new Slider((int)Q.Range, 125, (int)Q.Range)));
            ...
            #region Combo
            Config.SubMenu("R option").AddItem(new MenuItem("rCount", "范围内有地人数 自动 R").SetValue(new Slider(3, 0, 5)));
            Config.SubMenu("R option").AddItem(new MenuItem("afterGrab", "自动 R 后再 Q").SetValue(true));
            Config.SubMenu("R option").AddItem(new MenuItem("rKs", "R 抢人头").SetValue(false));
            Config.SubMenu("R option").AddItem(new MenuItem("inter", "打断技能")).SetValue(true);
