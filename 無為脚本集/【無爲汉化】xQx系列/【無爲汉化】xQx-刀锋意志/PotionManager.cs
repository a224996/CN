...
namespace Irelia
{
    internal class PotionManager
    {
        ...
        private void Load()
        {
            ...
            Program.MenuExtras.AddSubMenu(new Menu("药水模式", "PotionManager"));

            Program.MenuExtras.SubMenu("PotionManager").AddSubMenu(new Menu("红药", "Health"));
            Program.MenuExtras.SubMenu("PotionManager").SubMenu("Health").AddItem(new MenuItem("HealthPotion", "使用红药").SetValue(true));
            Program.MenuExtras.SubMenu("PotionManager").SubMenu("Health").AddItem(new MenuItem("HealthPercent", "最低血量%").SetValue(new Slider(30)));

            Program.MenuExtras.SubMenu("PotionManager").AddSubMenu(new Menu("蓝药", "Mana"));
