...
namespace Sophies_Soraka
{
    internal class SophiesSoraka
    {
        ...
        private static void CreateMenu()
        {
            ...
            // Drawing
            var drawingMenu = new Menu("范围", "ssDrawing");
            drawingMenu.AddItem(new MenuItem("drawQ", "显示 Q").SetValue(true));
            drawingMenu.AddItem(new MenuItem("drawW", "显示 W").SetValue(true));
            drawingMenu.AddItem(new MenuItem("drawE", "显示 E").SetValue(true));
            Menu.AddSubMenu(drawingMenu);

            // Misc
            var miscMenu = new Menu("杂项", "ssMisc");
            miscMenu.AddItem(new MenuItem("packets", "使用封包").SetValue(true));
            miscMenu.AddItem(new MenuItem("useQGapcloser", "Q 防突进").SetValue(true));
            miscMenu.AddItem(new MenuItem("useEGapcloser", "E 防突进").SetValue(true));
            miscMenu.AddItem(new MenuItem("autoW", "Auto 使用 W").SetValue(true));
            miscMenu.AddItem(new MenuItem("autoWPercent", "% 血量").SetValue(new Slider(50, 1)));
            miscMenu.AddItem(new MenuItem("autoWHealth", "自己血量").SetValue(new Slider(30, 1)));
            miscMenu.AddItem(new MenuItem("autoR", "Auto 使用 R").SetValue(true));
            miscMenu.AddItem(new MenuItem("autoRPercent", "% 血量").SetValue(new Slider(15, 1)));
            miscMenu.AddItem(new MenuItem("eInterrupt", "使用 E 打断技能").SetValue(true));
