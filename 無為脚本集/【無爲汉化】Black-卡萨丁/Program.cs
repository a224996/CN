...
namespace BlackKassadin
{
    internal class Program
    {
        ...
        {
            Menu = new Menu("【無爲汉化】Black-卡萨丁", "black" + ChampName, true);

            // Target selector
            var ts = new Menu("目标选择器", "ts");
            Menu.AddSubMenu(ts);
            ...
            // WaveClear
            var waveclear = new Menu("清兵", "waveclear");
            ...
            waveclear.AddItem(new MenuItem("wcActive", "清兵!").SetValue(new KeyBind('V', KeyBindType.Press)));
            ...
            misc.AddItem(new MenuItem("miscPacket", "使用封包").SetValue(true));
            misc.AddItem(new MenuItem("miscIgnite", "使用点燃").SetValue(true));
            misc.AddItem(new MenuItem("miscDFG", "使用冥火").SetValue(true));
            misc.AddItem(new MenuItem("miscUltToMouse", "大招往鼠标方向").SetValue(new KeyBind('G', KeyBindType.Press)));
            misc.AddItem(new MenuItem("miscUseR", "使用 R 跟随鼠标").SetValue(true));
            misc.AddItem(new MenuItem("miscUltStacks", "R极限被动层数")).SetValue(new Slider(3, 1, 4));


            //Damage after combo:
            var dmgAfterComboItem = new MenuItem("DamageAfterCombo", "绘制连招损伤").SetValue(true);
            ...
            // Drawings
            var drawings = new Menu("绘制", "drawings");
            ...
            drawings.AddItem(new MenuItem("drawRangeE", "E / R 范围").SetValue(new Circle(false, Color.Aquamarine)));
