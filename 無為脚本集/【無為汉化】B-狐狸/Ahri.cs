...
namespace AhriSharp
{
    ...
    internal class Ahri
    {
        ...
        public Ahri()
        {
            ...
            (_menu = new Menu("【無為汉化】B-狐狸", "AhriSharp", true)).AddToMainMenu();
            ...
            comboMenu.AddItem(new MenuItem("comboROnlyUserInitiate", "使用 R 只有手动启用").SetValue(false));
            ...
            farmMenu.AddItem(new MenuItem("farmStartAtLevel", "几级后才使用技能").SetValue(new Slider(8, 1, 18)));

            var drawMenu = _menu.AddSubMenu(new Menu("绘制", "Drawing"));
            drawMenu.AddItem(new MenuItem("drawQE", "显示 Q, E 范围").SetValue(new Circle(true, System.Drawing.Color.FromArgb(125, 0, 255, 0))));
            drawMenu.AddItem(new MenuItem("drawW", "显示 W 范围").SetValue(new Circle(false, System.Drawing.Color.FromArgb(125, 0, 0, 255))));
            var dmgAfterComboItem = new MenuItem("DamageAfterCombo", "显示连招伤害").SetValue(true); //copied from esk0r Syndra
