...
namespace KarthusRForRAWR
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            //Create the menu
            menu = new Menu("【無為汉化】死亡歌颂者", ChampionName, true);
            ...
            //Keys
            menu.AddSubMenu(new Menu("按键", "Keys"));
            menu.SubMenu("Keys")
                .AddItem(
            ...
            menu.SubMenu("Keys")
                .AddItem(
                    new MenuItem("HarassActiveT", "骚扰 (固定)!").SetValue(new KeyBind("Y".ToCharArray()[0],
                        KeyBindType.Toggle)));
            menu.SubMenu("Keys")
               .AddItem(
                   new MenuItem("wTar", "选择投掷W").SetValue(new KeyBind("W".ToCharArray()[0],
                       KeyBindType.Press)));
            menu.SubMenu("Keys")
                .AddItem(
                    new MenuItem("lastHitQ", "使用Q补兵").SetValue(new KeyBind("A".ToCharArray()[0],
                        KeyBindType.Press)));
            menu.SubMenu("Keys")
                .AddItem(
            ...
            //Spell Menu
            menu.AddSubMenu(new Menu("技能设置", "Spell"));
            //Q Menu
            menu.SubMenu("Spell").AddSubMenu(new Menu("Q设置", "QSpell"));
            menu.SubMenu("Spell").SubMenu("QSpell").AddItem(new MenuItem("qAA", "自动Q A兵的目标").SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Toggle)));
            menu.SubMenu("Spell").SubMenu("QSpell").AddItem(new MenuItem("qImmo", "自动Q 不动的目标").SetValue(true));
            menu.SubMenu("Spell").SubMenu("QSpell").AddItem(new MenuItem("qDash", "自动Q突进的目标").SetValue(true));
            //W
            menu.SubMenu("Spell").AddSubMenu(new Menu("W设置", "WSpell"));
            menu.SubMenu("Spell").SubMenu("WSpell").AddItem(new MenuItem("wTower", "敌人在塔下自动W").SetValue(true));
            menu.SubMenu("Spell").SubMenu("WSpell").AddItem(new MenuItem("wIfMana", "如果有蓝量自动放W").SetValue(true));
            //E
            menu.SubMenu("Spell").AddSubMenu(new Menu("E设置", "ESpell"));
            menu.SubMenu("Spell").SubMenu("ESpell").AddItem(new MenuItem("eManaCombo", "连招最小法力值").SetValue(new Slider(10, 0, 100)));
            menu.SubMenu("Spell").SubMenu("ESpell").AddItem(new MenuItem("eManaHarass", "骚扰最小法力值").SetValue(new Slider(70, 0, 100)));
            //R
            menu.SubMenu("Spell").AddSubMenu(new Menu("R设置", "RSpell"));
            menu.SubMenu("Spell").SubMenu("RSpell").AddItem(new MenuItem("rPing", "Ping可被击杀的敌人").SetValue(true));

            //Combo menu:
            menu.AddSubMenu(new Menu("连招", "Combo"));
            menu.SubMenu("Combo").AddItem(new MenuItem("selected", "重点选择目标").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("UseQCombo", "使用 Q").SetValue(true));
            menu.SubMenu("Combo").AddItem(new MenuItem("qHit", "Q 击中几率").SetValue(new Slider(3, 1, 4)));
            ...
            menu.SubMenu("Combo")
                .AddItem(new MenuItem("igniteMode", "模式").SetValue(new StringList(new[] { "连招", "可击杀" }, 0)));

            //Harass menu:
            menu.AddSubMenu(new Menu("骚扰", "Harass"));
            menu.SubMenu("Harass").AddItem(new MenuItem("UseQHarass", "使用 Q").SetValue(true));
            menu.SubMenu("Harass").AddItem(new MenuItem("qHit2", "Q 击中几率").SetValue(new Slider(3, 1, 4)));
            ...
            menu.SubMenu("Harass").AddItem(new MenuItem("minMana", "蓝量控制 >").SetValue(new Slider(60, 0, 100)));
            ...
            menu.SubMenu("Misc").AddItem(new MenuItem("UseInt", "使用 E 打断法术").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("UseGap", "使用 E 防止突进").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("packet", "使用封包").SetValue(true));
            menu.SubMenu("Misc").AddItem(new MenuItem("smartKS", "使用智能击杀").SetValue(true));

            //Damage after combo:
            MenuItem dmgAfterComboItem = new MenuItem("DamageAfterCombo", "绘制连招伤害").SetValue(true);
            ...
            //Drawings menu:
            menu.AddSubMenu(new Menu("显示范围", "Drawings"));
            ...
            menu.SubMenu("Drawings").AddItem(new MenuItem("drawUlt", "大招可击杀的").SetValue(true));
            menu.SubMenu("Drawings")
                .AddItem(dmgAfterComboItem);
				
			menu.AddSubMenu(new Menu("無為汉化", "by wuwei"));
			menu.SubMenu("by wuwei").AddItem(new MenuItem("qunhao", "汉化群：386289593"));
			menu.SubMenu("by wuwei").AddItem(new MenuItem("qunhao2", "娃娃群：13497795"));
            menu.AddToMainMenu();
            ...
            Game.PrintChat("銆愮劇鐐烘眽鍖栥€戞浜℃瓕棰傝€呪啋" + " 鎴愬姛鍔犺浇! --- by xSalice");
        }
