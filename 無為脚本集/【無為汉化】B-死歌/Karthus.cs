    ...
    * */
    ...
    internal class Karthus
    {
        ...
        public Karthus()
        {
            ...
            (_menu = new Menu("【無為汉化】B-死歌", "KarthusSharp", true)).AddToMainMenu();

            var targetSelectorMenu = new Menu("目标选择", "TargetSelector");
            ...
            comboMenu.AddItem(new MenuItem("comboAA", "使用 AA").SetValue(false));
            comboMenu.AddItem(new MenuItem("comboWPercent", "使用 W 蓝量 %").SetValue(new Slider(10)));
            comboMenu.AddItem(new MenuItem("comboEPercent", "使用 E 蓝量 %").SetValue(new Slider(15)));
            comboMenu.AddItem(new MenuItem("comboMove", "走砍/移动").SetValue(true));

            var harassMenu = _menu.AddSubMenu(new Menu("骚扰", "Harass"));
            harassMenu.AddItem(new MenuItem("harassQ", "使用 Q").SetValue(true));
            harassMenu.AddItem(new MenuItem("harassQPercent", "使用 Q 蓝量 %").SetValue(new Slider(15)));
            harassMenu.AddItem(new MenuItem("harassQLasthit", "优先补兵").SetValue(true));
            harassMenu.AddItem(new MenuItem("harassMove", "走砍/移动").SetValue(true));

            var farmMenu = _menu.AddSubMenu(new Menu("发育", "Farming"));
            farmMenu.AddItem(new MenuItem("farmQ", "使用 Q").SetValue(new StringList(new[] { "补兵", "清兵", "全部", "不用" }, 1)));
            farmMenu.AddItem(new MenuItem("farmE", "使用 E 清兵").SetValue(true));
            farmMenu.AddItem(new MenuItem("farmAA", "使用 AA 清兵").SetValue(false));
            farmMenu.AddItem(new MenuItem("farmQPercent", "使用 Q 蓝量 %").SetValue(new Slider(10)));
            farmMenu.AddItem(new MenuItem("farmEPercent", "使用 E 蓝量 %").SetValue(new Slider(20)));
            farmMenu.AddItem(new MenuItem("farmMove", "走砍/移动").SetValue(true));

            var notifyMenu = _menu.AddSubMenu(new Menu("通知R可杀敌人", "Notify"));
            notifyMenu.AddItem(new MenuItem("notifyR", "文本通知").SetValue(true));
            notifyMenu.AddItem(new MenuItem("notifyPing", "PING通知").SetValue(false));

            var drawMenu = _menu.AddSubMenu(new Menu("范围", "Drawing"));
            drawMenu.AddItem(new MenuItem("drawQ", "显示 Q 范围").SetValue(new Circle(true, System.Drawing.Color.FromArgb(125, 0, 255, 0))));

            var miscMenu = _menu.AddSubMenu(new Menu("杂项", "Misc"));
            miscMenu.AddItem(new MenuItem("ultKS", "大招击杀").SetValue(true));
            miscMenu.AddItem(new MenuItem("autoCast", "自动连招/清兵").SetValue(false));
