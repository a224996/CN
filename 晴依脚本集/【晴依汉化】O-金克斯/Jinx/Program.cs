...
namespace Jinx
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
			...
			Config.SubMenu("E Config").AddItem(new MenuItem("opsE", "敌人放技能时E").SetValue(true));
			...
			Config.SubMenu("R Config").AddItem(new MenuItem("hitchanceR", "高命中率才大招").SetValue(true));
			Config.SubMenu("R Config").AddItem(new MenuItem("useR", "半手动R").SetValue(new KeyBind('T', KeyBindType.Press))); //32 == space

            #endregion
            Config.AddItem(new MenuItem("Qtype", "全新Q逻辑").SetValue(false));
