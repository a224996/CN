...
namespace Jinx
{
    class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            SpellList.Add(R);
            SpellList.Add(R1);
			Config = new Menu("【晴依汉化】O-金克斯", ChampionName, true);
            ...
            #region E
                Config.SubMenu("W Config").AddItem(new MenuItem("Hit", "W 命中率").SetValue(new Slider(2, 3, 0)));
            ...
            #region E
                ...
                Config.SubMenu("E Config").AddItem(new MenuItem("comboE", "连招自动E[测试]").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("AGC", "自动E突进").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("opsE", "自动E对方引导技能的人").SetValue(true));
                Config.SubMenu("E Config").AddItem(new MenuItem("telE", "自动E位移").SetValue(true));
            #endregion
            #region R
                Config.SubMenu("R Config").AddItem(new MenuItem("autoR", "自动 R").SetValue(true));
                Config.SubMenu("R Config").AddItem(new MenuItem("hitchanceR", "高命中率自动R").SetValue(true));
                Config.SubMenu("R Config").AddItem(new MenuItem("useR", "半手动R").SetValue(new KeyBind('R', KeyBindType.Press))); //32 == space

            #endregion
            Config.SubMenu("Draw").AddItem(new MenuItem("noti", "显示提示").SetValue(false));
            Config.SubMenu("Draw").AddItem(new MenuItem("wRange", "显示 W 范围").SetValue(false));
            Config.SubMenu("Draw").AddItem(new MenuItem("onlyRdy", "当技能可用才显示范围").SetValue(true));
            Config.SubMenu("Draw").AddItem(new MenuItem("orb", "标记走砍目标").SetValue(true));
            Config.SubMenu("Draw").AddItem(new MenuItem("wTarget", "显示 W 目标").SetValue(true));
            Config.SubMenu("Draw").AddItem(new MenuItem("semi", "标记半手动R目标").SetValue(false));

            Config.AddItem(new MenuItem("pots", "自动吃药").SetValue(true));
            Config.AddItem(new MenuItem("debug", "调试").SetValue(false));
			Config.AddItem(new MenuItem("info1", "作者：sebastiank1"));
			Config.AddItem(new MenuItem("info2", "汉化：晴依"));
			Config.AddItem(new MenuItem("info3", "Roly Roly Poly"));
			...
			//Game.PrintChat("<font color=\"#ff00d8\">J</font>inx full automatic AI ver 2.6 <font color=\"#000000\">by sebastiank1</font> - <font color=\"#00BFFF\">Loaded</font>");
			Game.PrintChat("<font color=\"#00BFFF\">[閲戝厠鏂痌</font> <font color=\"#FFFFFF\">OneKeyToWin閲戝厠鏂浇鍏ャ€傚け涔愬洯锛屽氨鏄ū涔?</font>");
        }
