...
namespace UltimateCarry
{
	class Lucian : Champion
	{
		...
		private void LoadMenu()
		{
			...
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("useE_TeamFight_Range", "E 目标范围").SetValue(new Slider(1100, 2000, 500)));
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("useR_TeamFight", "使用 R").SetValue(false));
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("useR_TeamFight_2", "使用 R 如果在CD").SetValue(true));
			...
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("useE_LaneClear_Range", "E 小兵范围").SetValue(new Slider(1100, 2000, 500)));
			AddManaManager("LaneClear", 25);

			Program.Menu.AddSubMenu(new Menu("补兵", "LastHit"));
			Program.Menu.SubMenu("LastHit").AddItem(new MenuItem("useQ_LastHit", "使用 Q").SetValue(true));
