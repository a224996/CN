...
namespace UltimateCarry
{
	class Xerath : Champion 
	{
		...
		public Xerath()
        {
			...
			Game.OnSendPacket += Game_OnGameSendPacket2;
		...
		private void LoadMenu()
		{
			...
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("useE_Interupt", "使用 E 打断技能").SetValue(true));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("useR_KS", "使用 R 抢人头").SetValue((new KeyBind("T".ToCharArray()[0], KeyBindType.Press))));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("useR_safe", "安全范围").SetValue(new Slider(700, 2000, 0)));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("useR_Killabletext", "如果可以击杀聊天框提示").SetValue(true));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("useR_Killableping", "如果可以击杀ping提示").SetValue(true));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("useR_warning", "开启警告").SetValue(false));
			...
			switch(Program.Orbwalker.ActiveMode)
			{
				case Orbwalking.OrbwalkingMode.Combo:
					...
					if(Program.Menu.Item("useE_TeamFight").GetValue<bool>())
                        Cast_BasicLineSkillshot_Enemy(E, TargetSelector.DamageType.Magical);
					break;
