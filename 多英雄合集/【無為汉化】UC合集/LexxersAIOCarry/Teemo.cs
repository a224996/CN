...
namespace UltimateCarry
{
	class Teemo : Champion 
	{
		...
		private void LoadMenu()
		{
			...
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("useW_TeamFight", "如果敌人逃跑 使用 W").SetValue(true));
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("useR_TeamFight", "使用 R 在我身上").SetValue(true));
			...
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("put_Shroom1", "把蘑菇 VIP").SetValue(true));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("put_Shroom2", "把蘑菇 MIP").SetValue(false));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("put_Shroom3", "把蘑菇 LIP").SetValue(false));
			...
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_Shroom1", "绘制 蘑菇 VIP").SetValue(true));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_Shroom2", "绘制 蘑菇 MIP").SetValue(false));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_Shroom3", "绘制 蘑菇 LIP").SetValue(false));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_Vision", "蘑菇视觉").SetValue(new Slider(1500, 2500, 1000)));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("hint1", "VIP = 红"));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("hint2", "MIP = 橙色"));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("hint3", "LiP = 黄色"));
		...
		private void CastW()
		{
			if(!W.IsReady() || Q.IsReady())
				return;
            var target = TargetSelector.GetTarget(Orbwalking.GetRealAutoAttackRange(ObjectManager.Player) + 200, TargetSelector.DamageType.Physical);
