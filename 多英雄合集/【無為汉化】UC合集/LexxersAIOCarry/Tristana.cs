...
namespace UltimateCarry
{
	class Tristana : Champion
	{
		...
		private void LoadMenu()
		{
			...
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("useR_Interrupt", "使用 R 打断技能").SetValue(true));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("useR_Antigapclose", "使用 R 防突进").SetValue(true));

			Program.Menu.AddSubMenu(new Menu("抢人头", "KillSteal"));
			Program.Menu.SubMenu("KillSteal").AddItem(new MenuItem("use_ComboKS", "使用抢人头").SetValue(true));
			Program.Menu.SubMenu("KillSteal").AddItem(new MenuItem("use_ComboKS_W", "使用 W 抢人头").SetValue(true));
		...
		private bool ComboKs()
		{
			...
			foreach (var enemy in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsValidTarget() && !hero.IsAlly))
			{
				if(!enemy.IsValidTarget(W.Range + E.Range))
					continue;
				var targetdis = ObjectManager.Player.Distance(enemy.ServerPosition);
		...
		private float GetIgniteDamage(Obj_AI_Base target)
		{
			...
			if(smite == SpellSlot.Unknown)
				return 0;
			if(ObjectManager.Player.Spellbook.CanUseSpell(smite) == SpellState.Ready)
