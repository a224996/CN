...
namespace UltimateCarry
{
	class Azir : Champion
	{
		...
		public Azir()
		{
			LoadMenu();
			LoadSpells();
			Game.OnUpdate += Game_OnGameUpdate2;
		...
		private void LoadMenu()
		{
			...
			Program.Menu.SubMenu("Harass").AddItem(new MenuItem("useQ_Harass", "使用 Q").SetValue(new StringList(new[] { "Not", "at 1 Soldier", "at 2 Soldiers only" }, 1)));
			...
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_insec", "绘制元位置").SetValue(true));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_soldier", "绘制 士兵").SetValue(true));
		}
		...
		private void CastW()
		{
			...
			if (!ManaManagerAllowCast(W))
				return;
            var target = TargetSelector.GetTarget(W.Range + SoldierAttackRange, TargetSelector.DamageType.Magical);
		...
		internal static class Orbwalking
		{
			...
			Player.GetAutoAttackDamage(minion, true));
				...
				public Obj_AI_Base GetTarget()
				{
					...
					{
                        var target = TargetSelector.GetTarget(-1, TargetSelector.DamageType.Physical) ?? GetSoldierTargetHero();
						if(target != null)
							return target;
					}
