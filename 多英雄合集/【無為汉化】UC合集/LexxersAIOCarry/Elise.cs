...
namespace UltimateCarry
{
	class Elise : Champion 
	{
		...
		private void LoadMenu()
		{
			...
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("Sorry", "对不起，能让你"));
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("Sorry1", "禁用法术 :P"));
			
			Program.Menu.AddSubMenu(new Menu("骚扰", "Harass"));
			Program.Menu.SubMenu("Harass").AddItem(new MenuItem("useQ_Harass", "使用 Q 人形").SetValue(true));
			Program.Menu.SubMenu("Harass").AddItem(new MenuItem("useW_Harass", "使用 W 人形").SetValue(true));
			Program.Menu.SubMenu("Harass").AddItem(new MenuItem("useE_Harass", "使用 E 人形").SetValue(true));
			AddManaManager("Harass",40);

			Program.Menu.AddSubMenu(new Menu("清兵", "LaneClear"));
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("useQ_LaneClear", "使用 Q 人形").SetValue(true));
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("useW_LaneClear", "使用 W 人形").SetValue(true));
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("useE_LaneClear", "使用 E 人形").SetValue(true));
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("useQ_LaneClear2", "使用 Q 蜘蛛").SetValue(true));
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("useW_LaneClear2", "使用 W 蜘蛛").SetValue(true));
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("useE_LaneClear2", "使用 E 蜘蛛").SetValue(true));
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("LaneClear_forms", "Switch Forms").SetValue(true));
			Program.Menu.SubMenu("LaneClear").AddItem(new MenuItem("LaneClear_mana", "使用 蜘蛛 below Mana").SetValue(new Slider(40)));
			...
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_Q_Human", "绘制 Q 人形").SetValue(true));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_W_Human", "绘制 W 人形").SetValue(true));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_E_Human", "绘制 E 人形").SetValue(true));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_Q_Spider", "绘制 Q 蜘蛛").SetValue(true));
			Program.Menu.SubMenu("Drawing").AddItem(new MenuItem("Draw_E_Spider", "绘制 E 蜘蛛").SetValue(true));
		}
		...
		private void CheckSwitchtoHuman()
		{
			if (WSpider.IsReady())
				return;
            var target = TargetSelector.GetTarget(QSpider.Range, TargetSelector.DamageType.Magical);	
			if (target == null)
				Switchto("Human");
		}
		...
		private void CastEHuman()
		{
			if(!EHuman.IsReady())
				return;
            var target = TargetSelector.GetTarget(EHuman.Range, TargetSelector.DamageType.Magical);
		...
		private void CastWHuman()
		{
			if(!WHuman.IsReady())
				return;
            var target = TargetSelector.GetTarget(WHuman.Range, TargetSelector.DamageType.Magical);
		...
		private void CastQHuman()
		{
			if (!QHuman.IsReady())
				return;
            var target = TargetSelector.GetTarget(QHuman.Range, TargetSelector.DamageType.Magical);
