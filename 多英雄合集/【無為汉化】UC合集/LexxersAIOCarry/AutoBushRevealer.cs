...
namespace UltimateCarry
{
    class AutoBushRevealer
    {
        ...
        {
            _menu = Program.Menu.AddSubMenu(new Menu("自动扫眼", "AutoBushRevealer"));
            _menu.AddItem(new MenuItem("AutoBushEnabled", "启用").SetValue(true));
            _menu.AddItem(new MenuItem("AutoBushKey", "按键").SetValue(new KeyBind(Program.Menu.Item("Orbwalk").GetValue<KeyBind>().Key, KeyBindType.Press))); //32 == space

            var useWardsMenu = _menu.AddSubMenu(new Menu("使用眼: ", "AutoBushUseWards"));
