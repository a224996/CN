...
namespace UltimateCarry
{
	class Riven : Champion
	{
		...
		private void LoadMenu()
		{
			...
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("use_UC", "使用普通连招").SetValue(true));
			Program.Menu.Item("use_UC").ValueChanged += SwitchUc;
			Program.Menu.SubMenu("TeamFight").AddItem(new MenuItem("use_Detuks", "使用击杀连招").SetValue(false));
			...
			Program.Menu.AddSubMenu(new Menu("被动", "Passive"));
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("CancleQAnimation", "取消 Q 移动").SetValue(true));
			Program.Menu.Item("CancleQAnimation").ValueChanged += SwitchMove;
			Program.Menu.SubMenu("Passive").AddItem(new MenuItem("QLaugh", "取消 Q 大笑").SetValue(false));
		...
		private static void UseESmart(Obj_AI_Base target)
		{
			...
			var dist = ObjectManager.Player.Distance(target.ServerPosition);
        ...
        private static object UseRSmart(AttackableUnit attackableUnit)
        {
        ...
        private static void UseWSmart(AttackableUnit attackableUnit)
        {
		...
		private void CastE()
		{
			if(!E.IsReady())
				return;
            var target = TargetSelector.GetTarget(E.Range + Orbwalking.GetRealAutoAttackRange(Player), TargetSelector.DamageType.Physical);
			if(!target.IsValidTarget(E.Range + Orbwalking.GetRealAutoAttackRange(Player)) && (target.Distance(ObjectManager.Player.ServerPosition) >= Orbwalking.GetRealAutoAttackRange(Player)))
				return;
