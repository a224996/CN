...
namespace UltimateCarry
{
	class Champion
	{
		...
		{
			Program.Menu.AddSubMenu(new Menu("封包设置", "Packets"));
			Program.Menu.SubMenu("Packets").AddItem(new MenuItem("usePackets", "使用封包").SetValue(true));
		...
		public void Game_OnGameSendPacket(GamePacketEventArgs args)
        {
            ...
            if (decodedPacket.MoveType == 3 && !Program.Menu.Item("hitMinions").GetValue<bool>())
				args.Process = false;
		}

		public void AddManaManager(string menuname, int basicmana)
		{
			Program.Menu.SubMenu(menuname).AddItem(new MenuItem("ManaManager_" + menuname, "藍量").SetValue(new Slider(basicmana)));
        ...
        public Obj_AI_Hero Cast_BasicLineSkillshot_Enemy(Spell spell, TargetSelector.DamageType damageType = TargetSelector.DamageType.Physical)
		{
        ...
        public void Cast_BasicLineSkillshot_Enemy(Spell spell, Vector3 sourcePosition, TargetSelector.DamageType damageType = TargetSelector.DamageType.Physical)
		{
			if(!spell.IsReady() || !ManaManagerAllowCast(spell))
				return;
        ...
        public void Cast_BasicCircleSkillshot_Enemy(Spell spell, TargetSelector.DamageType damageType = TargetSelector.DamageType.Physical, float extrarange = 0)
		{
			if(!spell.IsReady() || !ManaManagerAllowCast(spell))
				return;
            var target = TargetSelector.GetTarget(spell.Range + extrarange, damageType);
			if(target == null)
				return;
        ...
        public void Cast_onEnemy(Spell spell, TargetSelector.DamageType damageType = TargetSelector.DamageType.Physical)
		{
			if(!spell.IsReady() || !ManaManagerAllowCast(spell))
				return;
            var target = TargetSelector.GetTarget(spell.Range, damageType);
        ...
        public void Cast_onMinion_nearEnemy(Spell spell, float range, TargetSelector.DamageType damageType = TargetSelector.DamageType.Physical, MinionTypes minionTypes = MinionTypes.All, MinionTeam minionTeam = MinionTeam.All)
		{
			if(!spell.IsReady() || !ManaManagerAllowCast(spell))
				return;
