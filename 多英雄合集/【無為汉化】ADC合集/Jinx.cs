...
namespace Marksman
{
    internal class Jinx : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("SwapDistance" + Id, "交换 Q 的距离").SetValue(true));
            config.AddItem(new MenuItem("SwapAOE" + Id, "交换 Q 进行AOE").SetValue(false));
            config.AddItem(new MenuItem("MinWRange" + Id, "最小 W 范围").SetValue(new Slider(525 + 65 * 2, 0, 1200)));
            config.AddItem(new MenuItem("AutoEI" + Id, "自动 E 不动的").SetValue(true));
            config.AddItem(new MenuItem("AutoES" + Id, "自动 E 减速的").SetValue(true));
            config.AddItem(new MenuItem("AutoED" + Id, "自动 E 嘲讽的 ").SetValue(false));
            config.AddItem(
                new MenuItem("CastR" + Id, "手动 R (2000 范围)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("ROverKill" + Id, "检查 R 是否伤害过度").SetValue(true));
            config.AddItem(new MenuItem("MinRRange" + Id, "最小 R 范围").SetValue(new Slider(300, 0, 1500)));
            config.AddItem(new MenuItem("MaxRRange" + Id, "最大 R 范围").SetValue(new Slider(1700, 0, 4000)));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawQBound" + Id, "显示 Q 状况").SetValue(
