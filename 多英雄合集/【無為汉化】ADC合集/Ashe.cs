...
namespace Marksman
{
    internal class Ashe : Champion
    {
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseWTH", "使用 W (切换)").SetValue(new KeyBind("H".ToCharArray()[0], KeyBindType.Toggle)));
            return true;
        }
        ...
        {
            config.AddItem(new MenuItem("DrawW" + Id, "W 范围").SetValue(new Circle(true, Color.CornflowerBlue)));
            config.AddItem(
                new MenuItem("DrawE" + Id, "E 范围").SetValue(new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(new MenuItem("DrawHarassToggleStatus", "绘制切换状态").SetValue(true));
            return true;
        }
