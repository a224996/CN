...
namespace Sharpshooter.Champions
{
    public static class Sivir
    {
        ...
        public static void Load()
        {
            ...
            SharpShooter.Menu.SubMenu("Misc").AddItem(new MenuItem("AutoQ", "被控住目标自动 Q", true).SetValue(true));
            SharpShooter.Menu.SubMenu("Misc").AddItem(new MenuItem("AutoE", "对目标的法术自动 E", true).SetValue(true));

            SharpShooter.Menu.SubMenu("Drawings").AddItem(new MenuItem("drawingAA", "真正的 AA 范围", true).SetValue(new Circle(true, Color.FromArgb(170,78,25))));
            SharpShooter.Menu.SubMenu("Drawings").AddItem(new MenuItem("drawingQ", "Q 范围", true).SetValue(new Circle(true, Color.FromArgb(170, 78, 25))));
        ...
        static void Orbwalking_OnAfterAttack(AttackableUnit unit, AttackableUnit target)
        {
            ...
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Mixed && SharpShooter.Menu.Item("harassUseW", true).GetValue<Boolean>() && Player.ManaPercentage() > SharpShooter.Menu.Item("harassMana", true).GetValue<Slider>().Value)
            {
                W.Cast();
                Orbwalking.ResetAutoAttackTimer();
            }
