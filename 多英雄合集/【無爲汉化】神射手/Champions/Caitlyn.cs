...
namespace Sharpshooter.Champions
{
    public static class Caitlyn
    {
        ...
        public static void Load()
        {
            ...
            SharpShooter.Menu.SubMenu("Misc").AddItem(new MenuItem("AutoW", "对被控制住目标 使用W", true).SetValue(true));
            SharpShooter.Menu.SubMenu("Misc").AddItem(new MenuItem("dash", "往鼠标方向 E", true).SetValue(new KeyBind('G', KeyBindType.Press)));

            SharpShooter.Menu.SubMenu("Drawings").AddItem(new MenuItem("drawingAA", "真正的 AA 范围", true).SetValue(new Circle(true, Color.FromArgb(255, 94, 0))));
