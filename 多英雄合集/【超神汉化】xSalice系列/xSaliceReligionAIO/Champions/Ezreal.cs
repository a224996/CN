    ...
    class Ezreal : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("热键", "Key");
            {
                ...
                key.AddItem(new MenuItem("R_Nearest_Killable", "R附近可杀",true).SetValue(new KeyBind("R".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("Force_R", "强行R",true).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Max_Range", "Q最大范围",true).SetValue(new Slider(1050, 500, 1200)));
                    qMenu.AddItem(new MenuItem("Auto_Q_Slow", "自动Q减速的",true).SetValue(true));
                    qMenu.AddItem(new MenuItem("Auto_Q_Immobile", "自动Q不动的",true).SetValue(true));
                ...
                {
                    wMenu.AddItem(
                        new MenuItem("W_Max_Range", "W最大范围",true).SetValue(new Slider(900, 500, 1050)));
                    spellMenu.AddSubMenu(wMenu);
                }

                var eMenu = new Menu("E", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_On_Killable", "可杀使用E",true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_On_Safe", "E安全检查",true).SetValue(true));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_Min_Range", "R最小范围",true).SetValue(new Slider(300, 0, 1000)));
                    rMenu.AddItem(new MenuItem("R_Max_Range", "R最大范围",true).SetValue(new Slider(2000, 0, 4000)));
                    rMenu.AddItem(new MenuItem("R_Mec", "R击中大于",true).SetValue(new Slider(3, 1, 5)));
            ...
            {
                miscMenu.AddItem(
                    new MenuItem("Misc_Use_WE", "向鼠标WE",true).SetValue(new KeyBind("T".ToCharArray()[0],
                        KeyBindType.Press)));
                //add to menu
