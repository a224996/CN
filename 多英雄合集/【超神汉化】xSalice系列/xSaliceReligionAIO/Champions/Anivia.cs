    ...
    class Anivia : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var spellMenu = new Menu("技能设置", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("qHit", "连招命中率",true).SetValue(new Slider(3, 1, 3)));
                    qMenu.AddItem(new MenuItem("qHit2", "骚扰命中率",true).SetValue(new Slider(3, 1, 4)));
                    qMenu.AddItem(new MenuItem("detonateQ", "自动引爆 Q",true).SetValue(true));
                    qMenu.AddItem(new MenuItem("detonateQ2", "Q 后面的敌人",true).SetValue(true));
                    spellMenu.AddSubMenu(qMenu);
                }

                var wMenu = new Menu("W", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("wallKill", "阻挡敌人去路",true).SetValue(true));
                    spellMenu.AddSubMenu(wMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("checkR", "自动关闭 R",true).SetValue(true));
            ...
            var farm = new Menu("清兵", "Farm");
            {
                ...
                farm.AddItem(new MenuItem("UseRFarm", "使用 R",true).SetValue(false));
            ...
            var draw = new Menu("绘制", "Drawings"); { 
                ...
                MenuItem drawComboDamageMenu = new MenuItem("Draw_ComboDamage", "绘制连招损伤",true).SetValue(true);
