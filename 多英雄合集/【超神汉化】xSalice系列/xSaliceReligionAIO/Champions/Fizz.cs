    ...
    class Fizz : Champion
    {
        ...
        private void LoadMenu()
        {
            ...
            var spellMenu = new Menu("技能", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Min_Dist", "最小距离使用 E", true).SetValue(new Slider(100, 1, 475)));
                    spellMenu.AddSubMenu(qMenu);
                }

                var eMenu = new Menu("E", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Min_Dist", "最小距离使用 E", true).SetValue(new Slider(100, 1, 400)));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {   
                    rMenu.AddItem(new MenuItem("rBestTarget", "R最好的目标", true).SetValue(new KeyBind("R".ToCharArray()[0], KeyBindType.Press)));
                    rMenu.AddItem(new MenuItem("R_Delay", "延迟释放R期间Q(毫秒)", true).SetValue(new Slider(100, 1, 500)));
                    rMenu.AddItem(new MenuItem("R_Max_Dist", "R 最大距离", true).SetValue(new Slider(1000, 200, 1300)));
                    rMenu.AddItem(new MenuItem("ROverkill", "R 过度检查", true).SetValue(false));
                    rMenu.AddItem(new MenuItem("AlwaysR", "总是使用R连招", true).SetValue(true));
            ...
            {
                combo.AddItem(new MenuItem("Combo_mode", "连招模式", true).SetValue(new StringList(new[] { "R-W-Q-E (正常)", "W-Q-R-E(R 期间 Q)", "R-E-W-Q (R->E 突进)" }, 1)));
                combo.AddItem(new MenuItem("Combo_Switch", "开关模式按键", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            ...
            var harass = new Menu("骚扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("turretCheck", "不在敌人塔下使用", true).SetValue(true));
                //add to menu
                menu.AddSubMenu(harass);
            }
            var farm = new Menu("清兵", "Farming");
            {
                ...
                farm.AddItem(new MenuItem("LaneClear_useE_minHit", "使用 E 至少命中", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            var drawMenu = new Menu("绘制", "Drawing");
            {
                ...
                drawMenu.AddItem(new MenuItem("Draw_Mode", "绘制模式", true).SetValue(true));
