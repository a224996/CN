...
namespace xSaliceReligionAIO.Champions
{
    class Karthus : Champion
    {
        ...
        {
            //Keys
            var keys = new Menu("热键", "Keys");
            {
                keys.AddItem(new MenuItem("ComboActive", "连招!", true).SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
                keys.AddItem(new MenuItem("HarassActive", "骚扰!", true).SetValue(new KeyBind("S".ToCharArray()[0], KeyBindType.Press)));
                keys.AddItem(new MenuItem("HarassActiveT", "骚扰 (切换)!", true).SetValue(new KeyBind("Y".ToCharArray()[0], KeyBindType.Toggle)));
                keys.AddItem(new MenuItem("wTar", "投掷 W ", true).SetValue(new KeyBind("W".ToCharArray()[0], KeyBindType.Press)));
                keys.AddItem(new MenuItem("lastHitQ", "Q 补兵", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                keys.AddItem(new MenuItem("LaneClearActive", "清兵!", true).SetValue(new KeyBind("X".ToCharArray()[0], KeyBindType.Press)));
                menu.AddSubMenu(keys);
            }

            //Spell Menu
            var spells = new Menu("法术", "Spell"); { 
                //Q Menu
                ...
                //W
                var wMenu = new Menu("W", "WSpell");
                {
                    wMenu.AddItem(new MenuItem("wTower", "敌人越塔自动 W", true).SetValue(true));
                    wMenu.AddItem(new MenuItem("wIfMana", "只要有法力值", true).SetValue(true));
                    spells.AddSubMenu(wMenu);
                }

                //E
                var eMenu = new Menu("E", "ESpell"); { 
                    eMenu.AddItem(new MenuItem("eManaCombo", "连招最少蓝量", true).SetValue(new Slider(10)));
                    eMenu.AddItem(new MenuItem("eManaHarass", "骚扰最少蓝量", true).SetValue(new Slider(70)));
                ...
                {
                    rMenu.AddItem(new MenuItem("rPing", "（本地ping）可击杀敌人", true).SetValue(true));
            ...
            {
                combo.AddItem(new MenuItem("selected", "重点选择的目标", true).SetValue(true));
                combo.AddItem(new MenuItem("UseQCombo", "使用 Q", true).SetValue(true));
                combo.AddItem(new MenuItem("qHit", "Q 击中几率", true).SetValue(new Slider(3, 1, 4)));
            ...
            var harass = new Menu("骚扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("qHit2", "Q 击中几率", true).SetValue(new Slider(3, 1, 4)));
                ...
                harass.AddItem(new MenuItem("minMana", "蓝量 >", true).SetValue(new Slider(60)));
            ...
            var misc = new Menu("杂项", "Misc");
            {
                ...
                misc.AddItem(new MenuItem("packet", "使用封包", true).SetValue(true));
                misc.AddItem(new MenuItem("smartKS", "使用聪明的击杀系统", true).SetValue(true));
            ...
            { 
                ...
                drawMenu.AddItem(new MenuItem("drawUlt", "绘制可用大招杀死", true).SetValue(true));
                MenuItem drawComboDamageMenu = new MenuItem("Draw_ComboDamage", "绘制连招伤害", true).SetValue(true);
                MenuItem drawFill = new MenuItem("Draw_Fill", "绘制补充伤害", true).SetValue(new Circle(true, Color.FromArgb(90, 255, 169, 4)));
        ...
        private void DrawEnemyKillable()
        {
                            ...
                            x => x.IsValidTarget()))
            {
                if (GetUltDmg(enemy) > enemy.Health - 30)
                {
                    ...
                    {
                        Packet.S2C.Ping.Encoded(new Packet.S2C.Ping.Struct(enemy.ServerPosition.X,enemy.ServerPosition.Y, 0, 0, Packet.PingType.Normal)).Process();
