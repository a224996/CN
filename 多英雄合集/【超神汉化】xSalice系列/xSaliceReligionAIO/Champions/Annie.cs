    ...
    class Annie : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("热键", "Key");
            {
                ...
                key.AddItem(new MenuItem("LastHitQ", "补兵使用 Q!", true).SetValue(new KeyBind("A".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("LastHitQToggle", "补兵使用 Q (切换)", true).SetValue(new KeyBind("J".ToCharArray()[0], KeyBindType.Toggle)));
                key.AddItem(new MenuItem("FlashKILL", "闪现击杀", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
                key.AddItem(new MenuItem("ChargeStun", "叠晕 (切换)!", true).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Toggle)));
                //add to menu
                menu.AddSubMenu(key);
            }
            ...
            var spellMenu = new Menu("技能设置", "SpellMenu");
            {
                ...
                {
                    qMenu.AddItem(new MenuItem("Q_Stun_LastHit", "使用Q晕", true).SetValue(false));
                    spellMenu.AddSubMenu(qMenu);
                }

                var wMenu = new Menu("W", "WMenu");
                {
                    wMenu.AddItem(new MenuItem("useW_enemyCount", "自动眩晕 人数>=", true).SetValue(new Slider(3, 1, 5)));
                    wMenu.AddItem(new MenuItem("W_charge", "使用W叠被动", true).SetValue(true));
                    spellMenu.AddSubMenu(wMenu);
                }

                var eMenu = new Menu("E", "EMenu");
                {
                    eMenu.AddItem(new MenuItem("E_Charge", "使用 E 叠被动", true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Block", "使用 E 抵挡攻击", true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Stun", "使用技能前开E", true).SetValue(true));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("useR_enemyCount", "自动 R 人数 >=, 6 = 关", true).SetValue(new Slider(3, 1, 6)));
                    rMenu.AddItem(new MenuItem("R_To_KS", "使用 R 抢人头", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("R_OverKill", "检测可击杀", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("R_Mode", "控制熊模式", true).SetValue(new StringList(new[] { "跟随目标", "跟着我" })));
            ...
            var combo = new Menu("连招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("R_Killable", "只用击杀使用R", true).SetValue(new KeyBind("H".ToCharArray()[0], KeyBindType.Toggle)));
                combo.AddItem(new MenuItem("useR_enemyCount_Combo", "R 眩晕人数 >=", true).SetValue(new Slider(3, 1, 5)));
                combo.AddItem(new MenuItem("FlashStun", "使用 闪现->R->眩晕击杀", true).SetValue(true));
                //add to menu
            ...
            var harass = new Menu("骚扰", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("Harass_Tower", "塔下骚扰", true).SetValue(true));
                AddManaManagertoMenu(harass, "Harass", 30);
                //add to menu
                menu.AddSubMenu(harass);
            }
            var farm = new Menu("清兵", "LaneClear");
            {
                farm.AddItem(new MenuItem("UseQFarm", "使用 Q", true).SetValue(true));
                farm.AddItem(new MenuItem("UseWFarm", "使用 W", true).SetValue(true));
                farm.AddItem(new MenuItem("LaneClear_useW_minHit", "使用 W 最少小兵", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            {
                misc.AddItem(new MenuItem("Interrupt", "使用晕中断法术", true).SetValue(true));
                misc.AddItem(new MenuItem("W_Gap", "使用晕防突进", true).SetValue(true));
                misc.AddItem(new MenuItem("smartKS", "智能击杀", true).SetValue(true));
                misc.AddItem(new MenuItem("chargeMana", "叠被动 蓝量 >=", true).SetValue(new Slider(30)));
                misc.AddItem(new MenuItem("chargeInFountain", "在泉水叠被动", true).SetValue(true));
                misc.AddItem(new MenuItem("disableAA", "当法术准备好了不AA", true).SetValue(false));
                menu.AddSubMenu(misc);
            }

            var drawMenu = new Menu("显示", "Drawing");
            {
                drawMenu.AddItem(new MenuItem("Draw_Disabled", "禁用全部", true).SetValue(false));
                drawMenu.AddItem(new MenuItem("Draw_Q", "显示 Q", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_W", "显示 W", true).SetValue(true));
                ...
                drawMenu.AddItem(new MenuItem("FlashStunTarget", "显示闪现击杀范围", true).SetValue(true));

                MenuItem drawComboDamageMenu = new MenuItem("Draw_ComboDamage", "显示连招伤害", true).SetValue(true);
                MenuItem drawFill = new MenuItem("Draw_Fill", "显示补偿伤害", true).SetValue(new Circle(true, Color.FromArgb(90, 255, 169, 4)));
