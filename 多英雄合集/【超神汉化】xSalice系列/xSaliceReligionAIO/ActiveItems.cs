...
namespace xSaliceReligionAIO
{
    class ActiveItems
    {
        ...
        public static void AddToMenu(Menu theMenu)
        {
            ...
            var offensiveItem = new Menu("进攻物品", "Offensive Items");
            {
            ...
            var summoners = new Menu("召唤师技能", "Summoners");
            {
                var ignite = new Menu("点燃", "Ignite");
                {
                    ignite.AddItem(new MenuItem("ignite", "使用点燃", true).SetValue(true));
                    ignite.AddItem(
                        new MenuItem("igniteMode", "点燃模式", true).SetValue(new StringList(new[] {"连招", "可击杀"})));
        ...
        private static void AddOffensiveItem(Menu subMenu, ActiveItems item)
        {
            var active = new Menu(item.ActiveName, item.ActiveName);
            {
                ...
                active.AddItem(new MenuItem(item.ActiveName + "dmgCalc", "增加伤害计算", true).SetValue(true));
                active.AddItem(new MenuItem(item.ActiveName + "killAble", "只有敌人可击杀使用", true).SetValue(false));
                active.AddItem(new MenuItem(item.ActiveName + "always", "总是使用", true).SetValue(item.Mode == 1 || item.Mode == 2));
                active.AddItem(new MenuItem(item.ActiveName + "myHP", "自己血量 <= %", true).SetValue(new Slider(25)));
                active.AddItem(new MenuItem(item.ActiveName + "enemyHP", "目标血量 <= %", true).SetValue(new Slider(50)));
