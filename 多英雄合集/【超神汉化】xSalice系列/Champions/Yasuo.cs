    ...
    class Yasuo : Champion
    {
        ...
        private void LoadMenu()
        {
            var key = new Menu("热键", "Key");
            {
                ...
                key.AddItem(new MenuItem("HarassActiveT", "骚扰 (切换)!", true).SetValue(new KeyBind("N".ToCharArray()[0], KeyBindType.Toggle)));
                ...
                key.AddItem(new MenuItem("Flee", "使用 E 逃跑", true).SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
                //add to menu
                menu.AddSubMenu(key);
            }

            var spellMenu = new Menu("技能设置", "SpellMenu");
            {
                var qMenu = new Menu("Q", "QMenu");
                {
                    qMenu.AddItem(new MenuItem("Q_Auto", "自动Q开关", true).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
                    qMenu.AddItem(new MenuItem("Q_Auto_third", "自动使用三段 Q", true).SetValue(true));
                    qMenu.AddItem(new MenuItem("Q_UnderTower", "塔下自动 Q", true).SetValue(false));
                    qMenu.AddItem(new MenuItem("Q_Stack", "切换自动叠加三段 Q", true).SetValue(new KeyBind("I".ToCharArray()[0], KeyBindType.Toggle)));
                    qMenu.AddItem(new MenuItem("Q_thirdE", "优先 E->三段 Q ->一段 Q", true).SetValue(true));
                ...
                {
                    //wind wall
                    var dangerous = new Menu("阻挡危险技能", "Dodge Dangerous");
                    {
                ...
                {
                    eMenu.AddItem(new MenuItem("E_Min_Dist", "最小距离使用 E", true).SetValue(new Slider(250, 1, 475)));
                    //e Evade
                    var dangerous = new Menu("Dodge Spells", "躲避法术");
                    {
                        foreach (var hero in ObjectManager.Get<Obj_AI_Hero>().Where(x => x.IsEnemy))
                        {
                            dangerous.AddSubMenu(new Menu(hero.ChampionName, hero.ChampionName));
                    ...
                    eMenu.AddItem(new MenuItem("E_GapClose", "使用 E 接近敌人", true).SetValue(true));
                    eMenu.AddItem(new MenuItem("E_Turret", "不要在炮塔下使用E", true).SetValue(new KeyBind("H".ToCharArray()[0], KeyBindType.Toggle)));
                    spellMenu.AddSubMenu(eMenu);
                }

                var rMenu = new Menu("R", "RMenu");
                {
                    rMenu.AddItem(new MenuItem("R_If_Killable", "R 如果敌人可击杀", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("delayR", "延迟 R 杀死敌人", true).SetValue(true));
                    rMenu.AddItem(new MenuItem("R_MEC", "自动 R 如果 >= 敌人数, 0 = 关闭", true).SetValue(new Slider(3, 0, 5)));
            ...
            var combo = new Menu("连招", "Combo");
            {
                ...
                combo.AddItem(new MenuItem("qHit", "Q 击中几率", true).SetValue(new Slider(2, 1, 3)));
                combo.AddItem(new MenuItem("UseECombo", "使用 E", true).SetValue(true));
                combo.AddItem(new MenuItem("UseRCombo", "使用 R", true).SetValue(true));
                combo.AddItem(new MenuItem("ComboR_MEC", "R 如果 >= 敌人数", true).SetValue(new Slider(3, 1, 5)));
                //add to menu
            ...
            var harass = new Menu("Harass", "Harass");
            {
                ...
                harass.AddItem(new MenuItem("qHit2", "Q 击中几率", true).SetValue(new Slider(2, 1, 3)));
                harass.AddItem(new MenuItem("UseEHarass", "使用 E", true).SetValue(true));
                //add to menu
                menu.AddSubMenu(harass);
            }
            var farm = new Menu("发育", "Farming");
            {
                farm.AddItem(new MenuItem("UseQLast", "使用 Q 补兵", true).SetValue(true));
                farm.AddItem(new MenuItem("UseELast", "使用 E 补兵", true).SetValue(true));
                farm.AddItem(new MenuItem("UseQFarm", "使用 Q 清兵", true).SetValue(true));
                farm.AddItem(new MenuItem("UseQ3Farm", "使用 Q3 清兵", true).SetValue(true));
                farm.AddItem(new MenuItem("UseEFarm", "使用 E 清兵", true).SetValue(true));
                farm.AddItem(new MenuItem("E_UnderTower_Farm", "E 塔下", true).SetValue(false));
                farm.AddItem(new MenuItem("LaneClear_useQ_minHit", "使用 Q 最少命中", true).SetValue(new Slider(2, 1, 6)));
                //add to menu
            ...
            {
                misc.AddItem(new MenuItem("smartKS", "使用智能击杀系统", true).SetValue(true));
                misc.AddItem(new MenuItem("Interrupt", "打断法术", true).SetValue(true));
                misc.AddItem(new MenuItem("predMode", "预测模式，, 关闭 = 快速, 打开 = 准确", true).SetValue(true));
                menu.AddSubMenu(misc);
            }

            var drawMenu = new Menu("绘制", "Drawing");
            {
                drawMenu.AddItem(new MenuItem("Draw_Disabled", "打开全部", true).SetValue(false));
                drawMenu.AddItem(new MenuItem("Draw_Q", "显示 Q", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_Q2", "显示 Q 扩展", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_E", "显示 E", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_R", "显示 R", true).SetValue(true));
                drawMenu.AddItem(new MenuItem("Draw_AutoQ", "显示模式", true).SetValue(true));
