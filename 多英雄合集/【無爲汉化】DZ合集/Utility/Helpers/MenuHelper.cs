...
namespace DZAIO.Utility.Helpers
{
    static class MenuHelper
    {
        ...
        {
            var mmMenu = new Menu("蓝量管理", "dzaio.champion."+DZAIO.Player.ChampionName.ToLowerInvariant()+".mm." + GetStringFromMode(mode));
            for (var i = 0; i < spellList.Count(); i++)
            {
                mmMenu.AddItem(
                    new MenuItem(
                        "dzaio.champion."+DZAIO.Player.ChampionName.ToLowerInvariant()+".manamanager."+GetStringFromSpellSlot(spellList[i]).ToLowerInvariant() + "最小蓝量" + GetStringFromMode(mode).ToLowerInvariant(),
            ...
            for (var i = 0; i < spellList.Count(); i++)
            {
                menu.AddItem(
                    new MenuItem(
                        ...
                        "使用 " + GetStringFromSpellSlot(spellList[i])).SetValue(values[i]));
            }
        }
        ...
        public static void AddDrawMenu(this Menu menu, Dictionary<SpellSlot,Spell> dictionary,Color myColor)
        {
            foreach (var entry in dictionary)
            {
                ...
                {
                    menu.AddItem(new MenuItem("dzaio."+ObjectManager.Player.ChampionName.ToLowerInvariant() + ".drawing.draw" + GetStringFromSpellSlot(slot), "显示" + GetStringFromSpellSlot(slot)).SetValue(new Circle(true, myColor)));
                }
            }
        }
        ...
        {
            menu.AddItem(
                    new MenuItem("dzaio.champion.customhitchance", "命中率").SetValue(
                        new StringList(new[] { "低", "正常", "高", "很高" }, 2)));
        }

        public static void AddNoUltiMenu(this Menu menu,bool allies)
        {
            var _menu = new Menu("不使用大招", "dzaio.champion."+DZAIO.Player.ChampionName.ToLowerInvariant()+".noult");
