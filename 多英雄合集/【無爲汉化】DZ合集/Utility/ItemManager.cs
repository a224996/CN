...
namespace DZAIO.Utility
{
    class ItemManager
    {
        ...
        public static void OnLoad(Menu menu)
        {
            ...
            var activatorMenu = new Menu(cName + " - 活化剂", "dzaio.activator");

            //Offensive Menu
            var offensiveMenu = new Menu("活化剂 - 进攻装备", "dzaio.activator.offensive");
            ...
            foreach (var item in offensiveItems)
            {
                ...
                itemMenu.AddItem(new MenuItem("dzaio.activator." + item.Id + ".always", "总是").SetValue(true));
                itemMenu.AddItem(new MenuItem("dzaio.activator." + item.Id + ".onmyhp", "我的血量 <  %").SetValue(new Slider(30)));
                itemMenu.AddItem(new MenuItem("dzaio.activator." + item.Id + ".ontghpgreater", "目标血量 >  %").SetValue(new Slider(40)));
                itemMenu.AddItem(new MenuItem("dzaio.activator." + item.Id + ".ontghplesser", "目标血量 <  %").SetValue(new Slider(40)));
                itemMenu.AddItem(new MenuItem("dzaio.activator." + item.Id + ".ontgkill", "在目标被杀死").SetValue(true));
                itemMenu.AddItem(new MenuItem("dzaio.activator." + item.Id + ".displaydmg", "显示损伤").SetValue(true));
            ...
            activatorMenu.AddItem(new MenuItem("dzaio.activator.activatordelay", "全局活化剂延迟").SetValue(new Slider(80, 0, 300)));
        ...
        {
            menu.AddItem(new MenuItem("dzaio.activator.customhitchance", "命中率").SetValue(new StringList(new[] { "低", "正常", "高", "很高" }, 2)));
        }
