...
namespace SigmaSeries.Plugins
{
    public class Rengar : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("UseItems", "使用物品").SetValue(true));
            config.AddItem(new MenuItem("sep", "-- 叠被动"));
            config.AddItem(new MenuItem("smart5Stack", "智能叠被动").SetValue(false));
            config.AddItem(new MenuItem("q1", "平A+Q.").SetValue(false));
            config.AddItem(new MenuItem("w1", "生命值小于X%使用Q").SetValue(false));
            config.AddItem(new MenuItem("wSlide", "生命值").SetValue(new Slider(15, 0, 100)));
            config.AddItem(new MenuItem("e1", "超出平A范围使用E").SetValue(false));
            config.AddItem(new MenuItem("sep2", "-- 非智能设置"));
            config.AddItem(new MenuItem("stackPriority", "5层被动优先").SetValue(new StringList(new[] { "Q", "W", "E" }, 0)));
            config.AddItem(new MenuItem("tripleQ", "手动Q").SetValue(new KeyBind("M".ToCharArray()[0], KeyBindType.Press)));
        }
        ...
        {
            config.AddItem(new MenuItem("sep22", "-- 控线"));
            ...
            config.AddItem(new MenuItem("sep33", "-- 清线"));
            config.AddItem(new MenuItem("51", "使用5层被动").SetValue(false));
            config.AddItem(new MenuItem("waveClearPriority", "5层被动优先").SetValue(new StringList(new[] { "Q", "W" }, 0)));
            ...
            config.AddItem(new MenuItem("sep", "-- 清野s"));
            config.AddItem(new MenuItem("JungleActive", "清野").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("52", "使用5层被动").SetValue(false));
            config.AddItem(new MenuItem("jungPriority", "5层被动优先").SetValue(new StringList(new[] { "Q", "W" }, 0)));
