...
namespace SigmaSeries.Plugins
{
    public class Varus : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("minQ", "Q最小层数").SetValue(new Slider(2, 0, 3)));
            config.AddItem(new MenuItem("UseECombo", "使用E").SetValue(true));
            config.AddItem(new MenuItem("minE", "E最小层数").SetValue(new Slider(2, 0, 3)));
        ...
        {
            config.AddItem(new MenuItem("useQFarm", "Q").SetValue(new StringList(new[] { "控线", "清线", "全部", "不" }, 3)));
