...
namespace UnderratedAIO.Champions
{
    class Sejuani
    {
        ...
        {
            config = new Menu("【無為汉化】猪妹", "Sejuani", true);
            // Target Selector
            ...
            menuD.AddItem(new MenuItem("drawaa", "显示 AA 范围")).SetValue(new Circle(true, Color.FromArgb(150, 150, 177, 208)));
            menuD.AddItem(new MenuItem("drawqq", "显示 Q 范围")).SetValue(new Circle(true, Color.FromArgb(150, 150, 177, 208)));
            menuD.AddItem(new MenuItem("drawww", "显示 W 范围")).SetValue(new Circle(true, Color.FromArgb(150, 150, 177, 208)));
            menuD.AddItem(new MenuItem("drawee", "显示 E 范围")).SetValue(new Circle(true, Color.FromArgb(150, 150, 177, 208)));
            menuD.AddItem(new MenuItem("drawrr", "显示 R 范围")).SetValue(new Circle(true, Color.FromArgb(150, 150, 177, 208)));
            ...
            menuC.AddItem(new MenuItem("useemin", "使用 E 最少")).SetValue(new Slider(1, 1, 5));
            menuC.AddItem(new MenuItem("useEminr", "E 最小范围")).SetValue(new Slider(250, 0, 900));
            menuC.AddItem(new MenuItem("useRmin", "R 最少人数")).SetValue(new Slider(1, 0, 5));
            menuC.AddItem(new MenuItem("useRminr", "大招最小范围")).SetValue(new Slider(0, 0, 350));
            menuC.AddItem(new MenuItem("manualR", "快速丢 R")).SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press));
            menuC.AddItem(new MenuItem("useItems", "使用物品")).SetValue(true);
            menuC.AddItem(new MenuItem("useIgnite", "使用点燃")).SetValue(true);
            
            config.AddSubMenu(menuC);
            // Clear/Jungle
            Menu menuJ = new Menu("清兵/清野 ", "jsettings");
            menuJ.AddItem(new MenuItem("useqC", "使用 Q")).SetValue(true);
            menuJ.AddItem(new MenuItem("usewC", "使用 W")).SetValue(true);
            menuJ.AddItem(new MenuItem("useeCmin", "使用 E 最少")).SetValue(new Slider(1, 1, 5));
            menuJ.AddItem(new MenuItem("useiC", "使用物品")).SetValue(true);
            menuJ.AddItem(new MenuItem("minmana", "保持X%的法力")).SetValue(new Slider(1, 1, 100));
            menuJ.AddItem(new MenuItem("useSmite", "使用惩戒")).SetValue(new Slider(1, 1, 100));
            config.AddSubMenu(menuJ);
            // Misc Settings
            Menu menuU = new Menu("杂项 ", "usettings");
            menuU.AddItem(new MenuItem("useqgc", "使用 Q 防突进")).SetValue(false);
            menuU.AddItem(new MenuItem("useqint", "使用 Q 打断技能")).SetValue(true);
            menuU.AddItem(new MenuItem("usergc", "使用 R 防突进")).SetValue(false);
            menuU.AddItem(new MenuItem("userint", "使用 R 打断技能")).SetValue(false);
            config.AddSubMenu(menuU);
            var sulti = new Menu("不给大招 ", "dontult");
