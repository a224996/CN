    ...
    class Orbwalker
    {
        ...
        public static void AddToMainMenu(Menu MainMenu)
        {
            ...
            var OWMenu = new Menu("走砍", "OW");
            {
                ...
                {
                    DrawMenu.AddItem(new MenuItem("OW_Draw_AARange", "平A范围").SetValue(new Circle(false, Color.FloralWhite)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_AARangeEnemy", "敌人平A范围").SetValue(new Circle(false, Color.Pink)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_HoldZone", "控制区域").SetValue(new Circle(false, Color.FloralWhite)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_HpBar", "小兵血格").SetValue(new Circle(false, Color.Black)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_HpBarThickness", "-> 血格密度").SetValue(new Slider(1, 1, 3)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_LastHit", "显示补兵").SetValue(new Circle(false, Color.Lime)));
                    DrawMenu.AddItem(new MenuItem("OW_Draw_NearKill", "附近可补小兵").SetValue(new Circle(false, Color.Gold)));
                    OWMenu.AddSubMenu(DrawMenu);
                }
                var MiscMenu = new Menu("杂项", "Misc");
                {
                    MiscMenu.AddItem(new MenuItem("OW_Misc_HoldZone", "防守").SetValue(new Slider(50, 0, 150)));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_FarmDelay", "补兵延迟").SetValue(new Slider(0, 0, 300)));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_MoveDelay", "动作迟缓").SetValue(new Slider(0, 0, 150)));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_ExtraWindUp", "额外结束时间").SetValue(new Slider(80, 0, 200)));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_AutoWindUp", "-> 自动设置").SetValue(true));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_PriorityUnit", "优先目标").SetValue(new StringList(new[] { "Minion", "Hero" })));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_MeleeMagnet", "合理延迟").SetValue(true));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_AllMovementDisabled", "禁用移动").SetValue(false));
                    MiscMenu.AddItem(new MenuItem("OW_Misc_AllAttackDisabled", "禁用攻击").SetValue(false));
                    OWMenu.AddSubMenu(MiscMenu);
                }
                var ModeMenu = new Menu("模式", "Mode");
                {
                    var ComboMenu = new Menu("连招", "OW_Combo");
                    {
                        ComboMenu.AddItem(new MenuItem("OW_Combo_Key", "热键").SetValue(new KeyBind(32, KeyBindType.Press)));
                        ComboMenu.AddItem(new MenuItem("OW_Combo_Move", "移动").SetValue(true));
                        ComboMenu.AddItem(new MenuItem("OW_Combo_Attack", "攻击").SetValue(true));
                        ModeMenu.AddSubMenu(ComboMenu);
                    }
                    var HarassMenu = new Menu("骚扰", "OW_Harass");
                    {
                        HarassMenu.AddItem(new MenuItem("OW_Harass_Key", "热键").SetValue(new KeyBind("V".ToCharArray()[0], KeyBindType.Press)));
                        HarassMenu.AddItem(new MenuItem("OW_Harass_Move", "移动").SetValue(true));
                        HarassMenu.AddItem(new MenuItem("OW_Harass_Attack", "攻击").SetValue(true));
                        HarassMenu.AddItem(new MenuItem("OW_Harass_LastHit", "补兵").SetValue(true));
                        ModeMenu.AddSubMenu(HarassMenu);
                    }
                    var ClearMenu = new Menu("清兵/清野", "OW_Clear");
                    {
                        ClearMenu.AddItem(new MenuItem("OW_Clear_Key", "按键").SetValue(new KeyBind("G".ToCharArray()[0], KeyBindType.Press)));
                        ClearMenu.AddItem(new MenuItem("OW_Clear_Move", "移动").SetValue(true));
                        ClearMenu.AddItem(new MenuItem("OW_Clear_Attack", "攻击").SetValue(true));
                        ModeMenu.AddSubMenu(ClearMenu);
                    }
                    var LastHitMenu = new Menu("补兵", "OW_LastHit");
                    {
                        LastHitMenu.AddItem(new MenuItem("OW_LastHit_Key", "热键").SetValue(new KeyBind(17, KeyBindType.Press)));
                        LastHitMenu.AddItem(new MenuItem("OW_LastHit_Move", "移动").SetValue(true));
                        LastHitMenu.AddItem(new MenuItem("OW_LastHit_Attack", "攻击").SetValue(true));
                        ModeMenu.AddSubMenu(LastHitMenu);
                    }
                    var FleeMenu = new Menu("逃跑", "OW_Flee");
                    {
                        FleeMenu.AddItem(new MenuItem("OW_Flee_Key", "按键").SetValue(new KeyBind("C".ToCharArray()[0], KeyBindType.Press)));
