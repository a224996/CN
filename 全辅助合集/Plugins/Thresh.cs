...
namespace Support.Plugins
{
    public class Thresh : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddBool("ComboQFollow", "使用 Q 跟随", true);
            config.AddBool("ComboW", "使用 W 接队友", true);
            config.AddBool("ComboE", "使用 E", true);
            config.AddSlider("ComboHealthE", "如果目标血量低于", 20, 1, 100);
            config.AddBool("ComboR", "使用 R", true);
            config.AddSlider("ComboCountR", "敌人数量使用大招", 2, 1, 5);
        }

        public override void HarassMenu(Menu config)
        {
            config.AddBool("HarassQ", "使用 Q", true);
            config.AddBool("HarassW", "使用 W 救队友", true);
            config.AddBool("HarassE", "使用 E", true);
            config.AddSlider("HarassHealthE", "如果目标血量低于", 20, 1, 100);
        }

        public override void MiscMenu(Menu config)
        {
            config.AddBool("Misc.Q.OnAttack", "对目标使用勾", true);
        }
        ...
        public override void InterruptMenu(Menu config)
        {
            ...
            config.AddBool("InterruptE", "使用 E 打断", true);
        }
